...
每一个用户池都有一个唯一的 **userPoolId**，比如「5cc59bdf1bbaf0188cce6d84」，这个 **userPoolId** 是不可修改的，主要用来标识你的用户池；除了 **userPoolId** ，每一个用户池还有一个 **secret**，**userPoolId** 和 **secret** 可以在代码层面授权开发者访问用户池内的用户数据（包含注册、登录、退出、修改资料、删除用户等操作）。
