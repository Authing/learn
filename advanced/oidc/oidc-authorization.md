...
需要请求的权限，如果需要获取 unionid 需要包含 unionid；如果需要**获取手机号**和 **email** 需要包含 phone email；如果需要用户与 Authing 之间的 Token 需要包含 authing\_token；如果需要 refresh\_token 需要包含 offline\_access。多个 scope 请用**空格分隔**。同时 **id\_token** 中会包含**相关的字段**。
{% endapi-method-parameter %}
...
| scope | 需要请求的权限，如果需要获取 unionid 需要包含 unionid；如果需要**获取手机号**和 **email** 需要包含 phone email；如果需要用户与 Authing 之间的 Token 需要包含 authing\_token；多个 scope 请用**空格分隔**。同时 id\_token 中会包含相关的字段。**隐式模式不能刷新 token**，所以 offline\_access 字段无效。 |
